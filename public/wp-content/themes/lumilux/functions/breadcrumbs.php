...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'lumilux' ),
			'archive'   => esc_html__( 'Archives', 'lumilux' ),
			'blog'      => esc_html__( 'Blog', 'lumilux' ),
			'search'    => esc_html__( 'Search results for', 'lumilux' ),
			'not_found' => esc_html__( 'Not Found', 'lumilux' ),
			'author'    => esc_html__( 'Author Archives:', 'lumilux' ),
			'day'       => esc_html__( 'Daily Archives:', 'lumilux' ),
			'month'     => esc_html__( 'Monthly Archives:', 'lumilux' ),
			'year'      => esc_html__( 'Yearly Archives:', 'lumilux' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'lumilux' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'lumilux' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'lumilux' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
