...
class Nova_Ajax_Search {
	...
	public function nova_get_search_results() {
		...
		if ( !empty( $products ) ) {
			...
			if (count($products) == 6) {
				$output['suggestions'].= '<a class="view-all" href="#">' . esc_html__('View All', 'lumilux') . '</a>';
			}
			wp_reset_postdata();

		} else {
			$output['suggestions'] =  '<div class="search-no-suggestions"><span class="search-st">' . esc_html__('No results', 'lumilux') . '</span></div>';
		}
		$output['suggestions'] = '<span class="product-search-heading">' . esc_html__("Search results", "lumilux") . '</span><div class="grid-x grid-padding-x grid-padding-y small-up-1 medium-up-2 large-up-3">' . $output['suggestions'] .'</div>';
